...
const svgPaths = {
  ...
  // The V (vertical-line) commands in these two paths arrive corrupted from some
  // copies of this component — they must be ASCII "V", or the path silently fails
  // to render in Chromium.
  p5113400:
...
function BrandBadge({ name = "Interfaces" }: { name?: string }) {
  return (
...
function SearchContainer({
  ...
  // Controlled when the caller passes a value, uncontrolled otherwise.
...
export interface MenuItemT {
  ...
  isActive?: boolean;
  children?: MenuItemT[];
  /** Runs instead of the default no-op when the row is clicked. */
  onSelect?: () => void;
}
...
const DEFAULT_NAV_ITEMS: NavItemT[] = [
...
/**
 * The rail on its own is a complete sidebar — `footer` replaces the avatar with
 * whatever the app wants to sit at the bottom, and `rounded` is turned off when
 * it is not paired with the detail pane.
 */
export function IconRail({
  ...
  return (
    <aside
      className={`bg-black flex flex-col gap-2 items-center p-4 w-16 h-full ${className}`}
    >
      {/* Logo */}
...
/** Kept for the paired layout below, which is what the demo renders. */
const IconNavigation = IconRail;
...
function DetailSidebar({
  ...
  return (
    <aside
      className={`bg-black flex flex-col gap-4 items-start p-4 rounded-r-2xl transition-all duration-500 h-full ${
        isCollapsed ? "w-16 min-w-16 !px-0 justify-center" : "w-80"
      }`}
      style={{ transitionTimingFunction: softSpringEasing }}
    >
      {!isCollapsed && <BrandBadge name={brandName} />}
...
export interface TwoLevelSidebarProps {
  /** Rail icons. Defaults to the sample navigation. */
  navItems?: NavItemT[];
  /** Controlled selection; falls back to internal state when omitted. */
  activeSection?: string;
  onSectionChange?: (section: string) => void;
  /** Second-level content for the selected rail item. */
  getContent?: (activeSection: string) => SidebarContent;
  brandName?: string;
  /** Replaces the account row at the bottom of the detail pane. */
  footer?: React.ReactNode;
  className?: string;
}
