...
async function adopt(target: SshTarget): Promise<GatewayFacts | null> {
  ...
  return runStep({ title: 'Setting a new admin token', note: 'Running as root on the server.' }, async (say) => {
    const result = await remoteScript(target, ADOPT_SCRIPT, say);
...
/**
 * Read the gateway's identity and give it a new admin token.
 *
 * Paths and the systemctl command come from the environment with the real values
 * as defaults, so this exact script can be exercised against a scratch config
 * without a server in front of it.
 */
const ADOPT_SCRIPT = `set -u
CONF=\${LT_CONF:-/etc/localtunnel/gateway.json}
